...
source("./src/CombRP_tmp.R")
# set.seed(345)
## 0 . this function for generating data like 4.2 paper j2ssb 2020
...
seeds = c(2134,57987,12450,378900)
...
for (iv in 1:1) {
  ...
  for (hi in 1:1) {
    ...
    pre_file_name = paste("main_logit_all_per_exp_a0s_200_package",hh[hi],"_",seeds[iv],format(Sys.time(), "%Y_%m_%d_%H_%M_%S"),sep="_")
...
# paramters for data generating eperiment 3
...
p=3000
func <- c("H23")      #magitute of mis
a_3 <- seq(0,0.6,length.out=length.a)
exp_para13 = expand.grid(n,func,p,a_3,rhos,nexps,KEEP.OUT.ATTRS = FALSE)
...
exp_para_exp_03_2 = rbind(exp_para13,exp_para14)
...
test_stat <- c("LST_pval_cauchy","LST_pval_single","LST_pval_hmp","LST_pval_beta_hat")
                      ...
                      .packages=c("MASS","glmnet","randomForest","log4r",
                                  "GRPtests","dplyr","pracma","tidyr","psych","harmonicmeanp")) %dopar% {
                                    ...
                                    for (j in 1:1) {
                                      ...
                                      proj_test = CombRP(y, x,family = fam, b0=h, np =n_alpha) 
